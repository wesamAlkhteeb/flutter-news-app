class CountryAbbreviathion {
  List<Map<String, String>> countriesJson = [
    ...
    {"country": "United Kingdom", "abbreviation": "UK"},
  ...
  List<CountryAbbreviathionModel> filterByAbbreviation(List<String> need) {
    need.sort();
    List<CountryAbbreviathionModel> result = [];
    for (int i = 0; i < need.length; i++) {
      int index = countriesJson.indexWhere(
          (element) => element["abbreviation"] == need[i].toUpperCase());
      if (index != -1) {
        result.add(CountryAbbreviathionModel(
            country: countriesJson[index]["country"]!,
            abbreviation: countriesJson[index]["abbreviation"]!));
      }
    }
    result.sort((m1,m2){
      return m1.country.compareTo(m2.country);
    });
    return result;
  }
}

class CountryAbbreviathionModel {
  String country, abbreviation;

  CountryAbbreviathionModel(
      {required this.country, required this.abbreviation});
}
